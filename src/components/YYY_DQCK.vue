<template>
  <div class="yyy_dqck">
    <h2 style="padding-top: 30px;margin-bottom: 30px;">资产提升月月赢+保有活动+<br>6~24月短定产品计算器</h2>
    ...
    <el-form :model="form" label-position="top" label-width="100vw" class="custom-form">
      <div class="item">
        ...
        <!--new_aum_to_deposit-->
        <el-form-item label="6-24个月短定产品购买金额（万元）"
                      prop="estimate_aum"
                      :rules="[
         { required: true, message: 'AUM不能为空'},
         {
          ...
          <el-input v-model="form.new_aum_to_deposit" style="width: 90vw;"></el-input>
        </el-form-item>

        <!--product_rate-->
        <el-form-item label="预计购买产品年化"
                      prop="estimate_aum"
                      :rules="[
         { required: true, message: '产品年化不能为空'},
         {
            validator: function(rule, value, callback) {
              if (/^(-?\d+)(\.\d+)?$/.test(value) == false) {
                callback('请输入AUM');
          ...
          <el-input v-model="form.product_rate" style="width: 90vw;"><template slot="append">%</template></el-input>
        </el-form-item>

        <!--product_date-->
        <el-form-item label="预计购买产品天数">
          <el-radio-group v-model="form.product_date">
      ...
      <div class="item">
        ...
        <el-form-item label="享受6-24个月短定产品达标收益" label-width="40vw">
          <el-input v-model="form.product_rights" type=“number” style="width: 90vw;" readonly="readonly"></el-input>
        </el-form-item>

        <el-form-item label="预计购买产品对应基准" label-width="40vw">
...
export default {
  ...
  methods: {
    ...
    onClick() {
      ...
      if (this.form.new_aum_to_deposit >= 600) {
        this.form.product_rights = 1200
      }else if (this.form.new_aum_to_deposit >= 400) {
        this.form.product_rights = 800
      }else if (this.form.new_aum_to_deposit >= 200) {
        this.form.product_rights = 400
      }else if (this.form.new_aum_to_deposit >= 100) {
        this.form.product_rights = 200
      }else if (this.form.new_aum_to_deposit >= 50) {
        this.form.product_rights = 100
      }else if (this.form.new_aum_to_deposit >= 20) {
        this.form.product_rights = 40
      }else if (this.form.new_aum_to_deposit >= 5) {
        this.form.product_rights = 10
      }else {
        this.form.product_rights = ''
        this.form.product_type = 'string'
      }
      ...
      var product = this.json_data.product.find(product => this.form.product_rights === product.id);
      if (product.stock !== 1) {
        this.form.product_rights = '已兑完'
        this.form.product_type = 'string'
      }

      var days = 0;
      if ('半年' === this.form.product_date) {
        days = 180;
      }else if ('1年' === this.form.product_date) {
        days = 360;
      }else if ('2年' === this.form.product_date) {
        days = 720;
      }
      this.form.deposit_rights = (this.form.new_aum_to_deposit * this.form.product_rate / 100 * days / 360 * 10000).toFixed(2);
      ...
      if ('string' !== this.form.product_type) {
        this.form.total_rights = this.form.total_rights * 1 + this.form.product_rights * 1;
      }
      this.form.total_rights = (this.form.total_rights * 1).toFixed(2);

      this.form.annual_rights_rate = (this.form.total_rights / this.form.new_aum_to_deposit / 10000 / days * 360 * 100).toFixed(2) + '%'
